...
// SAP action signature header (v2.4.0+)
inline constexpr const char* HTTP_HEADER_SAP_SIGNATURE = "X-Apple-ActionSignature";

// User-Agent for auth endpoint — Configurator, not iTunes
// Source: ipatool pkg/http/constants.go DefaultUserAgent
...
struct Sinf {
    ...
    std::vector<uint8_t> dpInfo; // macOS decryption key (from "dpInfo" plist key)
};
