...
PlistValue parse_value(XmlTokenizer& tok);

PlistDict parse_dict(XmlTokenizer& tok) {
    PlistDict d;
    while (!tok.at_end()) {
...
PlistArray parse_array(XmlTokenizer& tok) {
    PlistArray arr;
    while (!tok.at_end()) {
        std::string tag = tok.peek_tag();
        if (tag.empty()) {
...
PlistValue parse_value(XmlTokenizer& tok) {
    ...
    // This was the source of a real memory-exhaustion bug once: without this
    // check, a self-closing <array /> would fall through to parse_array(),
    // which would then read forward through the REST of the document looking
    // for a </array> that doesn't exist, building an unbounded array.
...
size_t istr_find(const std::string& haystack, const std::string& needle,
                 size_t pos = 0)
{
...
std::string extract_inner(const std::string& body, const std::string& tag) {
...
std::string extract_full(const std::string& body, const std::string& tag) {
    std::string open_pat = "<" + tag;
    size_t open_start = istr_find(body, open_pat);
    if (open_start == std::string::npos) return "";
...
std::string normalize_plist_body(const std::string& raw) {
...
// Top-level XML decoder: returns the root PlistDict
PlistDict decode_plist_xml(const std::string& raw) {
...
class BplistDecoder {
    ...
    PlistValue read_obj(uint64_t ref) const {
        ...
        if (hi == 0x4) {
            size_t n = lo;
            if (lo == 0xF) n = (size_t)read_int_at(pos);
    ...
    // pos points right after a count marker byte that had lo==0xF; the next
    // byte is an int marker.
    uint64_t read_int_at(size_t& pos) const {
