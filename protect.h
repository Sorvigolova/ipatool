...
//
// Sensitive fields (passwordToken, password, gsIdmsToken) are AES-GCM
// encrypted in RAM via SecureString. The encryption key is never stored —
// it's derived fresh on every encrypt/decrypt call:
...
// header and hwid.h (hwid.h doesn't need to know about SecureString, and
// this header doesn't need to know about hwid's platform-specific code).
//
...
// gsa.cpp would use a different (empty/default) key than SecureString::get()
// called later from main.cpp, and decryption would fail with
// "mem_decrypt: authentication failed". This bit us for real once the code
// was split from one giant main.cpp into gsa.cpp/appstore.cpp/main.cpp.
